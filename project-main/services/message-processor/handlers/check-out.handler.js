    ...
    try {
       ...
       RETURNING id`,
        ...
        if (result.success) {
            ...
            const checkInData = await db.query(
                'SELECT event_timestamp FROM time_stamps WHERE id = $1',
                [openCheckIn.id]
            );
            const startTime = DateTime.fromJSDate(new Date(checkInData.rows[0].event_timestamp), { zone: 'Europe/Athens' });
            ...
            // --- Έλεγχος υπερωρίας: αν checkout > adjusted_end + 10' ---
            // Adjusted end = scheduled_end + λεπτά καθυστέρησης check-in
            const today = openCheckIn.reference_date;
            const scheduleRow = await db.query(
                `SELECT ws.start_time, ws.end_time, ws.is_night_shift
            ...
            if (scheduleRow.rowCount > 0) {
                const ws = scheduleRow.rows[0];
                const schedStart = DateTime.fromISO(`${today}T${ws.start_time}`, { zone: 'Europe/Athens' });
                let schedEnd = DateTime.fromISO(`${today}T${ws.end_time}`, { zone: 'Europe/Athens' });
                if (ws.is_night_shift) schedEnd = schedEnd.plus({ days: 1 });

                const lateMinutes = Math.max(0, startTime.diff(schedStart, 'minutes').minutes);
                const adjustedEnd = schedEnd.plus({ minutes: lateMinutes });
                const overtimeMinutes = Math.round(now.diff(adjustedEnd, 'minutes').minutes);

                if (overtimeMinutes > 10) {
                    await notifier.sendMessage(platform, platformUserId, 'overtime_warning', {
