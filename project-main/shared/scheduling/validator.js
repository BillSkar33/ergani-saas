...
async function canEmployeeCheckIn(employeeId) {
    ...
    try {
       ...
       LIMIT 1`,
        ...
        // Μέγιστη αποδεκτή καθυστέρηση check-in: 60 λεπτά μετά την έναρξη βάρδιας
        const MAX_LATE_CHECKIN_MIN = 60;
        ...
        // Late marker: μετά από grace_after → αργοπορία (αλλά επιτρέπεται)
        const lateCheckInMark = shiftStart.plus({ minutes: graceAfter });
        // Μέγιστη καθυστέρηση: shift_start + 60 λεπτά → μετά απορρίπτεται
        const maxLateCheckIn = shiftStart.plus({ minutes: MAX_LATE_CHECKIN_MIN });
        ...
        // Αν πέρασε 1 ώρα από την έναρξη → δεν επιτρέπεται check-in
        if (now > maxLateCheckIn) {
            return {
                allowed: false,
                reason: 'too_late',
                message: `Έχεις υπερβεί το μέγιστο όριο καθυστέρησης (1 ώρα). Η βάρδια ξεκίνησε στις ${schedule.start_time.slice(0, 5)}.`,
            };
        }

        // Αν ήρθε μετά τη λήξη (εντός 60') → πιθανόν λάθος ημέρα
        if (now > shiftEnd) {
            return {
                allowed: false,
                reason: 'shift_ended',
        ...
        const isLate = now > lateCheckInMark;
        const lateMinutes = isLate ? Math.round(now.diff(shiftStart, 'minutes').minutes) : 0;

        return {
            allowed: true,
            reason: isLate ? 'late_arrival' : 'on_time',
            message: isLate ? `Αργοπορία (${lateMinutes} λεπτά)` : null,
            schedule: {
                name: schedule.name,
